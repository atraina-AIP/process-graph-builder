...
1. **Cross-device / server-side persistence.** `backend/main.py` writes graphs to a JSON store on disk (`load_store`/`save_store`, `backend/main.py:70-80`) and keeps `mutation_batches` + per-graph `versions` server-side (`mutate_graph`, `backend/main.py:281-298`). The frontend only persists to this browser's `localStorage`.
2. **A second copy of assist/mutate logic over the wire** — but note this is *not* a real LLM. `compile_assist_message` (`backend/main.py:136`) is the same kind of deterministic split-on-arrows stub as the frontend's `compileInstruction`. So today there is **no real LLM to lose** — the backend is a placeholder for a *future* server-side model router (see PROPEL_ALIGNMENT.md:37, "Move LLM assist behind a backend service").
