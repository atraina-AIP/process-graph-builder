...
The app is dependency-free and can also be opened directly from `app/static/index.html`.
...
This uses a classic file download/upload, so it works even when `app/static/index.html` is
opened directly from `file://`. The optional backend below is not required for any
of this.

Run the API and static frontend together:
...
- `app/static/styles.css` - product UI and canvas styling
- `app/static/app.js` - graph state, mutation engine, compiler stub, validation, and export
- `app/main.py` - FastAPI scaffold for graph get/mutate/assist/Markdown export
- `BACKLOG.md` - milestone backlog and test plan
- `PROPEL_ALIGNMENT.md` - Propel product, architecture, artifact, and naming guardrails
- `app/static/assets/process-graph-concept.png` - generated visual concept used for implementation direction

## MVP Notes

The local compiler is a deterministic browser-side stub. A FastAPI-compatible scaffold now exists under `app/` with `GET /graph/{id}`, `POST /graph/mutate`, `POST /graph/assist`, and `GET /graph/{id}/export/md`. The backend assist endpoint currently uses a deterministic fallback compiler so the wire shape is ready before an external LLM provider is configured.
