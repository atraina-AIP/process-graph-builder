...
backends — `JsonFileStore` (dev default) and `CosmosGraphStore` (graphs and mutation
batches partitioned by `/tenant_id`; Cosmos system fields stripped at the adapter layer).
...
- [x] Backend tests for mutate/get/assist/export — done (`app/tests/`, includes the storage-selection + JSON store tests). Cosmos backend needs a live account/emulator and is not yet covered.
